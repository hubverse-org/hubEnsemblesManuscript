...
pC <- ggplot(
  ...
  geom_point(
    data = cdf_defined_on_values |>
      dplyr::filter(distribution != "Q", value %in% lop_example_x),
    mapping = aes(color = distribution, size = distribution), shape = 15
  ) +
  coord_cartesian(xlim = c(75, 135)) +
...
#| fig-cap: "Example model output. One example forecast of weekly US incident influenza hospitalizations is shown for each of three models (panels). Forecasts are represented by a median (line) and 50% prediction interval (Q25-Q75)."
#| fig-width: 8
#| fig-height: 4
