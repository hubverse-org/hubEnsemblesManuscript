...
While the `hubExamples` package provides both corresponding target time series data (@tbl-example-target-time-series-data) and oracle output, we focus on the former which is convenient for making forecasts and plotting. The target time series data provides observed incident influenza hospitalizations in a given week for given location using columns `observation`, `date`, and `location`. The forecast-specific task ID variables `reference_date` and `horizon` are not relevant for this time series representation of the target data.
