...
The methods described in @sec-defs are implemented in the `hubEnsembles` package in a flexible, easy-to-use framework. Importantly, `hubEnsembles` is situated within the broader hubverse software infrastructure, which provides data standards and conventions for representing and working with model predictions. We begin with a short overview of hubverse concepts and conventions needed to utilize the `hubEnsembles` package, supplemented by example predictions provided by the hubverse, then explain the implementation of the two primary ensembling functions included in the package, `simple_ensemble()` and `linear_pool()`.

### Hubverse terminology and conventions

In the hubverse, model predictions are always represented in a standardized tabular format called "model output". Each row represents a single, unique prediction with each column providing information about what is being predicted, its scope, and its value. These columns will fall into one of four categories with the following purposes: (i) denote which model has produced the prediction (the "model ID"), (ii) provide details about what is being predicted (the "task IDs"), (iii) specify the type of prediction and identifying information (the "model output representation"), and (iv) give the value of the prediction itself [@hubverse_docs].

Each model's identifier given in the `model_id` column must be unique and is generally a concatenation of a short team name and model name using a hyphen ("-"). The task ID columns may vary in number and make-up to capture the details of the desired modeling outcome, as well as any conditions or assumptions used to generate the predictions [@hubverse_docs]. As an example, consider the modeling task of predicting weekly flu hospitalizations for different US locations. This information might be represented by task ID variables such as a `target` column with the value "wk inc flu hosp", a `location` column identifying location being predicted, a `reference_date` column of the "starting point" of the forecasts (not shown), and a `horizon` column of the number of weeks ahead into the future that the forecast is predicting for relative to the `reference_date` (@tbl-example-forecasts).
...
Both short-term forecasts and longer-term scenarios protections can be represented as model output but each may require different task ID columns for their respective modeling tasks. Consider scenario projections of incident COVID-19 cases in the US at different locations, for varying amounts of time in the future, and under different assumed conditions (like vaccination rates). Here, the task ID variables may consist of a `target` column of "inc case", a `location` column of the location being predicted (not shown), a `horizon` column specifying the date on which the projections were made (not shown), a `horizon` column of the number of steps ahead that the projection is being made for relative to the `origin_date`, and a `scenario_id` denoting the assumption of future conditions for which the model is making predictions. 

Different modeling efforts may use different sets of task ID columns with different values to define their prediction goals, or may simply choose distinct names to represent the same concept (as is the case with our two examples with `reference_date` and `origin_date` for a date task identifier). Some standard examples of task ID variables are available on the hubverse documentation website [@hubverse_docs].
...
#| data provided by the `hubExamples` package."
...
The concept of model output is codified by the `model_out_tbl` S3 class in the `hubUtils` package and is required by nearly every package in the hubverse, including `hubEnsembles`. Any tabular model predictions can be transformed to conform to the standards of this S3 class using the `as_model_out_tbl()` function from `hubUtils`. An example is provided in @sec-case-study.
...
The `hubExamples` package also provides corresponding target time series data (@tbl-example-target-time-series-data) and oracle output. Both contain the incident influenza hospitalizations in a given week for a given location but are used for different purposes. The target time series data is a more traditional representation of the observed "truth" in a time series format with columns `observation`, `date`, and `location`; this type of target data suited to be used as calibration data for generating forecasts. Oracle output, on the other hand, represents predictions that an "oracle model" would have generated had it known the observed values in advance. This format resembles that of model output data and is suited for evaluating the forecasts post hoc.
...
We can plot these forecasts and the target time series data using the `plot_step_ahead_model_output()` function from `hubVis`, another package for visualizing model outputs from the hubverse suite (@fig-plot-ex-mods). We subset the model output data and the target data to the location and time horizons we are interested in.
...
To further demonstrate the differences between the two ensembling functions and the utility of the `hubEnsembles` package, we examine a more complex example. Unlike the previous section's basic showcase of functionality to create multi-model ensembles, we use this case study to provide a more complete analysis that compares and evaluates ensemble model performance using real forecasts collected by a modeling hub, with an overarching goal of choosing a single best ensembling approach for the application.
...
All of the ensemble variations outperform the baseline model in this analysis, led by the quantile median which had the best scores overall for WIS, MAE, 50% PI coverage, and 95% PI coverage. However, other models sometimes demonstrated better performance, especially around the season's peak. This seems to indicate that the quantile median may not necessarily be the top-performing model for every influenza season.
