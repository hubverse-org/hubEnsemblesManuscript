...
In this section, we provide a simple example to illustrate the two main functions in [hubEnsembles]{.pkg}, `simple_ensemble()` and `linear_pool()`. The figures from here on are generated reproducibly using data from rds files stored in the `analysis/data/raw-data` directory and scripts in the `inst` directory.
