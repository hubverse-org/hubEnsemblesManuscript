...
hubEnsembles::example_target_data |>
  ...
  knitr::kable(col.names = c("`time_idx`", "`location`", "`value`"))
```
...
geometric_mean_ens <- hubEnsembles::example_model_output |>
...
weighted_mean_ens <- hubEnsembles::example_model_output |>
  hubEnsembles::simple_ensemble(
...
```{r}
#| prompt: true
linear_pool_ens <- hubEnsembles::example_model_output |>
...
target_data_plot <- hubEnsembles::example_target_data |>
  dplyr::filter(
    location == "US", 
    time_idx >= "2022-11-01", time_idx <= "2023-03-01"
  )
