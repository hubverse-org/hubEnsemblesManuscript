...
This computes the average value of predictions across different models for each fixed quantile level $\theta$. For a normal distribution or any distribution with a shape and scale parameter, the resulting quantile average will be the same type of distribution, with shape and scale parameters that are the average of the shape and scale parameters from the individual distributions (@fig-example-quantile-average-and-linear-pool, panel B) **\[ELR: Should this refer to location and scale parameterws rather than shape and scale parameters? \]**. In other words, this method interprets the predictive probability distributions that are being combined as uncertain estimates of a single true distribution. It is also possible to use other combination functions, such as a weighted median, to combine quantile predictions.

The probability average or linear pool is calculated by averaging probabilities across predictions for a fixed value of the target variable, $x$. In other words, for a set $\mathcal{F} = \{F_i(x)| i \in 1,...,N \}$ containing the values of CDFs at the point $x$ **\[ELR: There is some inconsistency in how we describe the set $\mathcal{F}$ here and how we describe $\mathcal{Q}$ above. Here we say that $\mathcal{F}$ is a set of values of CDFs, while above we say that $\mathcal{Q}$ is a set of quantile *functions*. I think either is ok, but we should pick whether we want both to be functions (quantile functions or cumulative distribution functions) or we want both to be sets of function values (quantiles or probabilities). \]** and weights $\pmb{w}$, the CDF for a linear pool is calculated as
...
The hubverse also provides standards for target data, which can be stored in one of two formats: target time series or oracle output. The two tabular representations differ in terms of columns and purposes. Target time series data is a more traditional representation of the observed "truth" in a time series format with minimal columns; this format usually serves as calibration data for generating forecasts. Oracle output, on the other hand, represents prediction that an "oracle model" would have made had it known the observed values in advance. This format resembles model output data and is suited for evaluating forecasts. Some examples of target data are given in @sec-simple-ex.
...
The model output is stored in a data object named `forecast_outputs` and contains mean, median, quantile, and sample forecasts of future incident influenza hospitalizations, as well as CDF and PMF forecasts of hospitalization intensity. Each prediction is made for **\[ELR: Light suggestion: instead of "made for", consider "described by" or "identified by"? \]** five task ID variables: the location for which the forecast was made (`location`), the date on which the forecast was made (`reference_date`), the number of steps ahead (`horizon`), the date of the forecast prediction (`target_end_date`, a combination of the date the forecast was made and the forecast horizon), and the forecast target (`target`). We begin by examining the predictions for weekly incident influenza hospitalizations, displaying a subset of each output type in @tbl-example-model-outputs. **\[ELR: 
Should we add an illustration of sample outputs in this table? Or update the description here to say specifically the mean, median, and quantile output types? \]**
...
Each unique prediction from an ensemble model is scored against target data using the `score_forecasts()`[^2] function from the `covidHubUtils` package, as a hubverse package for scoring and evaluation has not yet been fully implemented. **\[ELR: We do now have hubEvals available and able to compute the four metrics used in this investigation. I don't have a strong feeling about whether it's worth it to take the time to use it. It would be nice to demonstrate the use of this package as part of this paper, and advertise the availability of the package. I'd defer to Nick and others on whether the time it would require to update the analysis would be worth it. \]** This function outputs each of the metrics described above. We use median forecasts taken from the 0.5 quantile for the MAE evaluation.
...
**\[ELR: In this figure, it would be good to use the same colors for each model in the top left panel (all models) as in the other panels.  For example, right now it looks like maybe the green color is used for the baseline in the "all models" panel, but is used for the median-ensemble in the lower right panel. \]**
...
**\[ELR: Low-priority comments on figures 8 and 9: (1) Suggest capitalizing MAE and WIS in the vertical axis labels. (2) is the figure unreadable if you don't cut off the large scores? If so, keep what you have now. If not, it's nice to show all the scores. \]**
