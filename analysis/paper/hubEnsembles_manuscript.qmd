...
Forecasts must conform to hubverse standards to be fed into either of the ensembling functions, so we first transform the raw forecasts using the  `as_model_out_tbl()` function [^1] from the [hubUtils]{.pkg} package. Here, we specify the task ID variables `forecast_date` (when the forecast was made), `location`, `horizon`, and `target`.
...
flu_forecasts_hubverse <- flu_forecasts_raw |>
    ...
    dplyr::mutate(target_end_date = 
                  ceiling_date(forecast_date + weeks(horizon), "weeks") - days(1)) |>
    as_model_out_tbl(
      model_id_col = "model",
