...
The component forecasts used to generate the ensembles consist of those submitted during the 2021-2022 and 2022-2023 influenza seasons, the only two complete seasons for which FluSight collected quantile forecasts for a target of weekly incident flu hospitalizations at the time of this writing. Over the course of these two seasons, twenty-six modeling teams made forecasts for 53 total dates, 54 locations in the United States, 4 horizons, 1 target, and 23 quantiles using 44 different models (including the baseline and median ensemble models run by the FluSight team). In the 2021-2022 season 23 models made predictions for 22 weeks spanning from late January 2022 to late June 2022 while the 2022-2023 season saw 18 models making predictions for 31 weeks spanning mid-October 2022 to mid-May 2023. Both seasons forecasted for the same locations (the 50 states, Washington DC, Puerto Rico, the Virgin Islands, and the US as a whole), horizons (1 to 4 weeks ahead), quantiles (Q = {.010, 0.025, .050, .100, ..., .900, .950, .990}), and target (week ahead incident flu hospitalization).

Our only inclusion criteria for the component forecasts was that every prediction (defined by a unique combination of task IDs date, location, horizon, and target) was made for all 23 required quantiles and that they were not generated by the FluSight baseline or median ensemble. In practice, this meant that no forecasts made by outside teams were excluded, matching the real-time fluctuation in number of models included in the median ensemble run by FluSight.

Since these predictions were queried from zoltar, which has its own formatting standards, they do not conform to those for the hubverse. However, like most tabular-form forecasts, the forecasts are easily transformed using the `as_model_out_tbl()` function from the `hubUtils` package. Below we print the first six rows of the transformed component forecasts made on May 15, 2023 for California.

```{r transform data, eval=TRUE}
flu_forecasts_raw <- readr::read_rds(
  here::here("analysis/data/raw_data/flu_forecasts_raw.rds")
)
...
forecast_data_hub <- dplyr::filter(
  forecast_data_hub,
...
lp_normal <- hubEnsembles::linear_pool(forecast_data_hub,
  weights = NULL,
  n_samples = 1e5, model_id = "lp-normal",
  tail_dist = "norm"
)

lp_lognormal <- hubEnsembles::linear_pool(forecast_data_hub,
  weights = NULL,
  n_samples = 1e5,
