...
future\textsuperscript{7--11} \textbf{{[}EH: suggestions on references
...
modeling teams into a collaborative ``hub''\textsuperscript{12}, where
...
influenza\textsuperscript{10,13}, dengue\textsuperscript{14}, West Nile
virus\textsuperscript{15}, and more recently
SARS-CoV-2\textsuperscript{16--18}.
...
(Figure~\ref{fig-example-quantile-average-and-linear-pool}, panel B)
\textbf{{[}ELR: Should this refer to location and scale parameterws
rather than shape and scale parameters? {]}}. In other words, this
method interprets the predictive probability distributions that are
being combined as uncertain estimates of a single true distribution. It
is also possible to use other combination functions, such as a weighted
median, to combine quantile predictions.
...
CDFs at the point \(x\) \textbf{{[}ELR: There is some inconsistency in
how we describe the set \(\mathcal{F}\) here and how we describe
\(\mathcal{Q}\) above. Here we say that \(\mathcal{F}\) is a set of
values of CDFs, while above we say that \(\mathcal{Q}\) is a set of
quantile \emph{functions}. I think either is ok, but we should pick
whether we want both to be functions (quantile functions or cumulative
distribution functions) or we want both to be sets of function values
(quantiles or probabilities). {]}} and weights \(\pmb{w}\), the CDF for
a linear pool is calculated as
