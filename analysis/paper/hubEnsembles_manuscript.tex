...
respectively. For sample predictions, those with the same
\texttt{output\_type\_id} from a particular model may be assumed to
correspond to a single sample from a joint distribution. This unique
property originates from the fact that the sample output type may
capture dependence in the model to outcomes across levels of task ID
variables, unlike the other output types that only summarize the
marginal predictive distribution for single combination of modeled task
id variables.
...
displaying only a subset of each in
...
Flusight-baseline & wk inc flu hosp & 1 & mean & NA & 582 \\
...
MOBS-GLEAM\_FLUH & wk inc flu hosp & 1 & mean & NA & 705 \\
...
PSI-DICE & wk inc flu hosp & 1 & mean & NA & 594 \\
