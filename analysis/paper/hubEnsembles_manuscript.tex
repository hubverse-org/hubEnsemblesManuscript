...
\begin{tcolorbox}[enhanced jigsaw, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Keywords and definitions: Concepts in statistical forecasting}, leftrule=.75mm, colback=white, opacityback=0, rightrule=.15mm, left=2mm, colbacktitle=quarto-callout-tip-color!10!white, toptitle=1mm, arc=.35mm, coltitle=black, bottomrule=.15mm, breakable, titlerule=0mm, opacitybacktitle=0.6, colframe=quarto-callout-tip-color-frame, bottomtitle=1mm, toprule=.15mm]
...
multiple levels of the task ID
variables.)}\label{tbl-model-output-rep}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\texttt{output\_type}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\begin{tcolorbox}[enhanced jigsaw, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Keywords and definitions: Terminology from the hubverse}, leftrule=.75mm, colback=white, opacityback=0, rightrule=.15mm, left=2mm, colbacktitle=quarto-callout-tip-color!10!white, toptitle=1mm, arc=.35mm, coltitle=black, bottomrule=.15mm, breakable, titlerule=0mm, opacitybacktitle=0.6, colframe=quarto-callout-tip-color-frame, bottomtitle=1mm, toprule=.15mm]
...
\includegraphics{hubEnsembles_manuscript_files/figure-pdf/fig-plot-ex-mod-samps-1.pdf}
...
influenza hospitalizations. The ranking of the models differed slightly
among the four scoring metrics; the two linear pools perform the best of
the ensembles in terms of WIS and 95\% prediction interval coverage
while the quantile median has slightly better MAE and the best 50\%
coverage for ensembles. The quantile mean, however, displayed the worst
values for every metric out of the ensembles, particularly with the MAE.
Note that the 2022-2023 season was especially difficult to forecast due
to its unusual timing and large peak (see
Figure~\ref{fig-plot-forecasts-hubVis} and
Figure~\ref{fig-wis-vs-forecast-date}), resulting in overall MAE values
close to that of the baseline for even the best ensembles. However, all
the ensembles almost always outperform the baseline model, despite this
being obscured by a few weeks of very poor performance when summarizing
evaluation metrics over two seasons
(Figure~\ref{fig-wis-vs-forecast-date}).

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.3333}}
...
and consequently the highest coverage rates. This is useful for the 95\%
prediction intervals, which better capture the fluctuations surrounding
the especially large peak of the 2022-2023 season than any other models;
however, it also led to 50\% interval coverage that nearly always
exceeded the nominal level. The linear pools' substantially better 95\%
coverage rates are likely the source of their low WIS values in the
overall-evaluation.

The median ensemble, which has the best MAE and 50\% interval coverage
of the ensembles, tended to have slightly better median (point)
forecasts and mid-width intervals. While these intervals are properly
calibrated outside of the large peak, they fail to appropriately capture
uncertainty during that time, which resulted in WIS values higher than
that of the linear opinion pools. The quantile mean's interval widths
vary throughout both seasons, though it usually has narrower intervals
than the linear pools. However, this model's median forecasts have a
...
line). For simplicity, only forecasts from select four-week periods
during the 2022-2023 influenza season are shown.}

\end{figure}%

Averaging across all time points, the linear pools have the best scores
for WIS and 95\% prediction interval coverage and top performance for
MAE. (Note that we only show plots of WIS vs forecast date, faceted by
season and horizon, as it is our principal metric. The 95\% interval
coverage plots display similar trends, and model performance is nearly
identical among the ensembles for MAE.) Every ensemble tends to
substantially outperform the baseline model for WIS and MAE, except at
the top of the 2022-2023 season peak when the baseline had much lower
values for both metrics (see Figure~\ref{fig-wis-vs-forecast-date}). The
linear pools demonstrated the strongest performance compared to the
other models in the 2022-2023 season at both 1 week and 4 week ahead
horizons for WIS and 95\% coverage. This seems to indicate that their
wide intervals provided the greatest utility during times of rapid
change when other models remained overconfident in their forecasts.

However, the linear pools are not always the top ensemble models. There
are a few weeks where the quantile average ensembles are slightly better
in terms of WIS. In general, the median ensemble has the best WIS values
for the 1 week ahead horizon during the 2021-2022 season, and the best
MAE for both seasons' 1 week ahead forecasts. The quantile mean also
performs the best out of the ensembles at the 4 week ahead horizon for
the 2021-2022 season (Figure~\ref{fig-wis-vs-forecast-date}). Depending
on the current forecasting conditions, different models may make more
appropriate or accurate predictions.
...
While usually unsuited for short-term forecasting, the linear pools'
conservative wide intervals were suitable for making predictions for
influenza season with an unusually large peak. Conversely, the LOP
models did not perform as well during the milder season without any
periods of rapid change; here, the quantile median and mean ensembles
demonstrated better, more robust performance. Thus, while the linear
opinion pools offer better overall performance in this analysis thanks
to their correct assessment of uncertainty during periods of rapid
change, there are clearly also settings in which other ensemble methods
...
tails showcase top-performance for a range of metrics, including
weighted interval score, mean absolute error, and prediction interval
coverage, particularly when outlying component forecasts are likely more
important. Yet, the other ensemble methods we tested also showcased a
clear improvement over the baseline model and even outperformed the
linear pools during periods of greater stability and less variation. The
accuracy gains from ensemble models motivate the use of a ``hub-based''
approach to prediction for infectious diseases, public health, and in
other fields.
