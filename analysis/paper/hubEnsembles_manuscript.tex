...
specifically on the functionality encompassed in \texttt{hubEnsembles}.
We provide an overview of the methods implemented, including
mathematical definitions and properties (Section~\ref{sec-defs}) as well
as implementation details (Section~\ref{sec-implementation}), a basic
...
quantile average will be the same distribution, and the shape and scale
parameters will be the average of the shape and scale parameters from
...
needed to utilize the \texttt{hubEnsembles} package, supplemented by
example predictions provided by the hubverse, then explain the
implementation of the two ensembling functions included in the package,
...
identifying the location being predicted, a \texttt{reference\_date}
column with the ``starting point'' of the forecasts (not shown), and a
\texttt{horizon} column with the number of steps ahead that the forecast
is predicting relative to the \texttt{reference\_date}
...
provided by the \texttt{hubExamples} package.}
...
hubverse documentation website (Consortium of Infectious Disease
Modeling Hubs 2024).
...
output type is not yet directly supported by the \texttt{hubEnsembles}
package and should be converted to another output type before computing
an ensemble). The \texttt{output\_type\_id} provides additional
...
are not yet supported by the \texttt{hubEnsembles} package. Adapted from
...
functionality of the \texttt{hubEnsembles} package (Consortium of
Infectious Disease Modeling Hubs 2024). This example hub was generated
with modified forecasts from the FluSight forecasting challenge, which
will be discussed in detail in Section~\ref{sec-case-study}. The example
hub includes both example model output data and target data (sometimes
known as ``truth'' data), which are stored in the \texttt{hubExamples}
package as data objects named \texttt{forecast\_outputs} and
\texttt{forecast\_target\_ts}. Note that the toy model outputs contain
...
\texttt{hubExamples} package (Table~\ref{tbl-example-target-data}). The
...
\SpecialCharTok{+}\NormalTok{     )}
\SpecialCharTok{\textgreater{}}\NormalTok{ hubVis}\SpecialCharTok{::}\FunctionTok{plot\_step\_ahead\_model\_output}\NormalTok{(}
...
To further demonstrate the utility of the \texttt{hubEnsembles} package
and the differences between the two ensembling functions, we examine a
more complex example. Unlike the previous section's basic showcase of
...
The \texttt{hubEnsembles} package is situated within the larger hubverse
...
When using \texttt{hubEnsembles}, it is important to carefully choose an
...
Ongoing development of the \texttt{hubEnsembles} package and the larger
