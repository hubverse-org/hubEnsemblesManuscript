...
SMPC \And Consortium of Infectious Disease Modeling
Hubs\thanks{A list of authors and their affiliations appears at the end of the paper.} \AND Nicholas
...
Hochheiser, Anna Krystalli, Consortium of Infectious Disease Modeling
Hubs, Nicholas G. Reich, Evan L. Ray} %% comma-separated
...
\hypertarget{hubverse-terminology-and-conventions}{%
...
\hypertarget{tbl-flu-forecasts}{}
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.2588}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.2588}}
  ...
  >{\raggedleft\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.0706}}@{}}
\toprule\noalign{}
...
\hypertarget{tbl-example-scenarios}{}
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1928}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1205}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.0964}}
  ...
  >{\raggedleft\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1807}}
...
hubverse\citep{hubverse_docs}.}\tabularnewline
\end{longtable}
...
type is not yet supported by the \pkg{hubEnsembles} package). The
...
\hypertarget{tbl-model-output-rep}{}
\begin{longtable}[]{@{}
...
\caption{\label{tbl-model-output-rep}A table summarizing how the model
output representation columns are used for predictions of different
output types. Adapted from \citep{hubverse_docs}}\tabularnewline
\end{longtable}

\hypertarget{sec-ens-fns}{%
...
\hypertarget{tbl-fns-by-output-type}{}
\begin{longtable}[]{@{}
...
\caption{\label{tbl-fns-by-output-type}Summary of ensemble function
calculations for each output type. The ensemble function (columns)
determines the operation that is performed, and in the case of
probabilistic output types (\texttt{quantile}, \texttt{cdf},
\texttt{pmf}), this also determines what ensemble distribution is
generated (quantile average, \(F_{Q}^{-1}(\theta)\), or linear pool,
\(F_{LOP}(x)\)). The resulting ensemble will be returned in the same
output type as the inputs. Thus, the output type (rows) determines how
the resulting ensemble distribution is summarized (as a quantile
function, \(F^{-1}(\theta)\), cumulative distribution function,
\(F(x)\), or probability mass function \(f(x)\)). Estimating individual
model cumulative probabilities is required to compute a
\texttt{linear\_pool()} for predictions of \texttt{quantile} output
type; see Section~\ref{sec-linear-pool} for details. In the case of
\texttt{simple\_ensemble()}, we report the calculations for the default
case where \texttt{agg\_fun\ =\ "mean"}; however, if another aggregation
function is chosen (e.g., \texttt{agg\_fun\ =\ "median"}), that
calculation would be performed instead. For example,
\texttt{simple\_ensemble(...,\ agg\_fun\ =\ "median")} applied to
predictions of \texttt{mean} output type would return the median of
individual model means.}\tabularnewline
\end{longtable}

\hypertarget{sec-linear-pool}{%
...
{\centering \includegraphics{hubEnsembles_manuscript_files/figure-pdf/fig-example-quantile-average-and-linear-pool-1.pdf}
...
\hypertarget{example-data-a-forecast-hub}{%
...
\pkg{hubEnsembles} package as data objects named
...
\hypertarget{tbl-example-forecasts}{}
\begin{longtable}[]{@{}
...
{hubEnsembles} package and is a subset of the
\texttt{example-complex-forecast-hub} data provided by the hubverse
\citep{hubverse_docs}.}\tabularnewline
\end{longtable}

We also have corresponding target data included in the
\pkg{hubEnsembles} package (Table~\ref{tbl-example-target-data}). The
...
\hypertarget{tbl-example-target-data}{}
\begin{longtable}[]{@{}llr@{}}
\toprule\noalign{}
...
{\centering \includegraphics{hubEnsembles_manuscript_files/figure-pdf/fig-plot-ex-mods-1.pdf}
...
\hypertarget{tbl-example-forecasts-pmf}{}
\begin{longtable}[]{@{}
...
example data is provided in the {hubEnsembles} package and is a subset
of the \texttt{example-complex-forecast-hub} data provided by the
hubverse \citep{hubverse_docs}.}\tabularnewline
\end{longtable}

\begin{figure}

{\centering \includegraphics{hubEnsembles_manuscript_files/figure-pdf/fig-plot-ex-mods-pmf-1.pdf}
...
\hypertarget{creating-ensembles-with-simple_ensemble}{%
...
the \texttt{value} column is rounded to two digits.}\tabularnewline
\end{longtable}

\hypertarget{changing-the-aggregation-function}{%
...
{\centering \includegraphics{hubEnsembles_manuscript_files/figure-pdf/fig-plot-ensembles-1.pdf}
...
\hypertarget{weighting-model-contributions}{%
...
\hypertarget{creating-ensembles-with-linear_pool}{%
\subsection{Creating ensembles with
...
{\centering \includegraphics{hubEnsembles_manuscript_files/figure-pdf/fig-plot-ex-quantile-and-linear-pool-1.pdf}
...
from the \pkg{hubUtils} package. Here, we specify the task ID variables
...
\hypertarget{tbl-case-study-flu-forecasts}{}
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.2588}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.0941}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.2588}}
...
have been omitted for brevity.}\tabularnewline
\end{longtable}
...
\hypertarget{performance-results-across-ensembles}{%
...
\hypertarget{tbl-overall-evaluation}{}
\begin{longtable}[]{@{}
...
the metric values are often quite similar among the
...
R> flu_dates_off_season <- as.Date("2022-06-27") + weeks(0:15)
R> all_flu_dates <- c(flu_dates_21_22, flu_dates_22_23)
R> 
R> select_dates <- c(all_flu_dates[seq(1, 69, 4)], flu_dates_21_22[22] +
...
{\centering \includegraphics{hubEnsembles_manuscript_files/figure-pdf/fig-plot-forecasts-hubVis-1.pdf}
...
{\centering \includegraphics{hubEnsembles_manuscript_files/figure-pdf/fig-mae-vs-forecast-date-1.pdf}
...
{\centering \includegraphics{hubEnsembles_manuscript_files/figure-pdf/fig-wis-vs-forecast-date-1.pdf}
...
{\centering \includegraphics{hubEnsembles_manuscript_files/figure-pdf/fig-cov95-vs-forecast-date-1.pdf}
...
\pkg{hubEnsembles}, it is important to carefully choose an ensemble
...
efforts, the \pkg{hubEnsembles} package provides important software
...
\hypertarget{consortium-of-infectious-disease-modeling-hubs}{%
