...
from these models (referred to as ``model output'' from here on) being
combined will vary based on the setting. For example, some public health
...
it has been proposed ot use short-term forecasts of incidence to inform
...
begin with a short overview of hubverse concepts and conventions needed
to utilize the \texttt{hubEnsembles} package, supplemented by example
predictions provided by the hubverse, then explain the implementation of
the two primary ensembling functions included in the package,
...
In the hubverse, model predictions are always represented in a
standardized tabular format called ``model output''. Each row represents
a single, unique prediction with each column providing information about
what is being predicted, its scope, and its value. These columns will
fall into one of four categories with the following purposes: (i) denote
which model has produced the prediction (the ``model ID''), (ii) provide
details about what is being predicted (the ``task IDs''), (iii) specify
the type of prediction and identifying information (the ``model output
representation''), and (iv) give the value of the prediction
itself\textsuperscript{29}.

Each model's identifier given in the \texttt{model\_id} column must be
unique and is generally a concatenation of a short team name and model
name using a hyphen (``-''). The task ID columns may vary in number and
make-up to capture the details of the desired modeling outcome, as well
as any conditions or assumptions used to generate the
predictions\textsuperscript{29}. As an example, consider the modeling
task of predicting weekly flu hospitalizations for different US
locations. This information might be represented by task ID variables
such as a \texttt{target} column with the value ``wk inc flu hosp'', a
\texttt{location} column identifying location being predicted, a
\texttt{reference\_date} column of the ``starting point'' of the
forecasts (not shown), and a \texttt{horizon} column of the number of
weeks ahead into the future that the forecast is predicting for relative
to the \texttt{reference\_date} (Table~\ref{tbl-example-forecasts}).
...
\texttt{forecast\_outputs} data provided by the \texttt{hubExamples}
package.}
...
Both short-term forecasts and longer-term scenarios protections can be
represented as model output but each may require different task ID
columns for their respective modeling tasks. Consider scenario
projections of incident COVID-19 cases in the US at different locations,
for varying amounts of time in the future, and under different assumed
conditions (like vaccination rates). Here, the task ID variables may
consist of a \texttt{target} column of ``inc case'', a \texttt{location}
column of the location being predicted (not shown), a \texttt{horizon}
column specifying the date on which the projections were made (not
shown), a \texttt{horizon} column of the number of steps ahead that the
projection is being made for relative to the \texttt{origin\_date}, and
a \texttt{scenario\_id} denoting the assumption of future conditions for
which the model is making predictions.

Different modeling efforts may use different sets of task ID columns
with different values to define their prediction goals, or may simply
choose distinct names to represent the same concept (as is the case with
our two examples with \texttt{reference\_date} and \texttt{origin\_date}
for a date task identifier). Some standard examples of task ID variables
are available on the hubverse documentation website\textsuperscript{29}.
...
\texttt{scenario\_outputs} data provided by the \texttt{hubExamples}
package.}
...
The concept of model output is codified by the \texttt{model\_out\_tbl}
S3 class in the \texttt{hubUtils} package and is required by nearly
every package in the hubverse, including \texttt{hubEnsembles}. Any
tabular model predictions can be transformed to conform to the standards
of this S3 class using the \texttt{as\_model\_out\_tbl()} function from
\texttt{hubUtils}. An example is provided in
...
summary of samples), and other ensemble methods may then be utilized on
the alternate output type.
...
In this section, we provide a simple example to illustrate and compare
the simple classes multi-model ensembles, using two main functions in
\texttt{hubEnsembles}, \texttt{simple\_ensemble()} and
\texttt{linear\_pool()}.
...
functionality of the \texttt{hubEnsembles} package\textsuperscript{29}.
...
model output data and two formats of target data (sometimes known as
``truth'' data), which are stored in the \texttt{hubExamples} package as
data objects named \texttt{forecast\_outputs},
\texttt{forecast\_target\_ts}, and \texttt{forecast\_oracle\_output}.
Note that this model outputs data contain only a small subset of
predictions for select dates, locations, and output type IDs, far fewer
...
The \texttt{hubExamples} package also provides corresponding target time
series data (Table~\ref{tbl-example-target-time-series-data}) and oracle
...
Cauchy using the \texttt{tail\_dist} argument\textsuperscript{46}. This
choice usually does not have a large impact on the resulting ensemble
...
\texttt{distfromq} package at \url{https://reichlab.io/distfromq/}.)
...
To further demonstrate the differences between the two ensembling
functions and the utility of the \texttt{hubEnsembles} package, we
examine a more complex example. Unlike the previous section's basic
showcase of functionality to create multi-model ensembles, we use this
case study to provide a more complete analysis that compares and
evaluates ensemble model performance using real forecasts collected by a
modeling hub, with an overarching goal of choosing a single best
ensembling approach for the application.
...
through a collaborative challenge called FluSight\textsuperscript{48}.
...
ensembles from Zoltar\textsuperscript{49}, a repository designed to
...
function from the \texttt{hubUtils} package. Here, we specify the task
ID variables \texttt{forecast\_date} (when the forecast was made),
...
absolute error (MAE), weighted interval score (WIS)\textsuperscript{50},
